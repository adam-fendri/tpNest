...
import { pageDTO } from 'src/todo/dto/pageDTO';
import { SearchDTO } from 'src/todo/dto/searchDTO';
import { StatusDTO } from 'src/todo/dto/statusDTO';
import { TodoAddDTO } from 'src/todo/dto/todoAddDTO';
import { TodoUpdateDTO } from 'src/todo/dto/todoUpdateDTO';
...
private readonly todoRepository: Repository<TodoEntity>) {}
  ...
  async restoreSection(id: string) {
    ...
    const sectionToRecover = await this.todoRepository.findOne({where:{id}});
  ...
  async searchBydescANDstatus(search:SearchDTO,page: pageDTO): Promise<TodoEntity[]>{
   ...
   if(search.critere!=undefined){
    queryBuilder.where("todo.description like :critere ", { critere:`%${search.critere}%`})
    .orWhere("todo.name like :critere ", { critere:`%${search.critere}%`});
   }
